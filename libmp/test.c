...
testvecdigmuladd(char *str, mpdigit d)
{
	...
	if(loops > 1)
		HOSTED_API(print)("%lld ns for a %d*%d vecdigmul\n", (nsec()-now)/loops, b->top*Dbits, Dbits);
	mpnorm(b2);
	HOSTED_API(print)("0 + %B * %ux = %B\n", b, d, b2);
...
testmul(char *str)
{
	...
	for(i = 0; i < loops; i++)
		mpmul(b, b1, b2);
	if(loops > 1)
		HOSTED_API(print)("%lld µs for a %d*%d mult\n", (nsec()-now)/(loops*1000),
			b->top*Dbits, b1->top*Dbits);
	HOSTED_API(print)("%B * %B = %B\n", b, b1, b2);
...
testdigdiv(char *str, mpdigit d)
{
	...
	for(i = 0; i < loops; i++)
		mpdigdiv(b->p, d, &q);
	if(loops > 1)
		HOSTED_API(print)("%lld ns for a %d / %d div\n", (nsec()-now)/loops, 2*Dbits, Dbits);
	HOSTED_API(print)("%B / %ux = %ux\n", b, d, q);
	mpfree(b);
}
...
testdiv(mpint *x, mpint *y)
{
	...
	for(i = 0; i < loops; i++)
		mpdiv(x, y, b2, b3);
	if(loops > 1)
		HOSTED_API(print)("%lld µs for a %d/%d div\n", (nsec()-now)/(1000*loops),
			x->top*Dbits, y->top*Dbits);
	HOSTED_API(print)("%B / %B = %B %B\n", x, y, b2, b3);
	mpfree(b2);
	mpfree(b3);
}
...
testmod(mpint *x, mpint *y)
{
	...
	for(i = 0; i < loops; i++)
		mpmod(x, y, r);
	if(loops > 1)
		HOSTED_API(print)("%lld µs for a %d/%d mod\n", (nsec()-now)/(1000*loops),
			x->top*Dbits, y->top*Dbits);
	HOSTED_API(print)("%B mod %B = %B\n", x, y, r);
	mpfree(r);
}
...
testinvert(mpint *x, mpint *y)
{
	...
	for(i = 0; i < loops; i++)
		mpinvert(x, y, r);
	if(loops > 1)
		HOSTED_API(print)("%lld µs for a %d in %d invert\n", (nsec()-now)/(1000*loops),
			x->top*Dbits, y->top*Dbits);
	HOSTED_API(print)("%B**-1 mod %B = %B\n", x, y, r);
	mpmul(r, x, d1);
	mpmod(d1, y, d2);
	HOSTED_API(print)("%B*%B mod %B = %B\n", x, r, y, d2);
	mpfree(r);
	mpfree(d1);
	mpfree(d2);
}
...
testexp(char *base, char *exp, char *mod)
{
	...
	for(i = 0; i < loops; i++)
		mpexp(b, e, m, res);
	if(loops > 1)
		HOSTED_API(print)("%ulldµs for a %d to the %d bit exp\n", (nsec()-now)/(loops*1000),
...
testgcd(void)
{
	...
	HOSTED_API(print)("gcd %B*%B+%B*%B = %B?\n", a, x, b, y, d);
	mpfree(a);
	mpfree(b);
	...
	for(i = 0; i < loops; i++){
		...
		mpadd(t1, t2, t2);
		if(mpcmp(d, t2) != 0)
			HOSTED_API(print)("%d gcd %B*%B+%B*%B != %B\n", i, a, x, b, y, d);
//		else
//			HOSTED_API(print)("%d euclid %B*%B+%B*%B == %B\n", i, a, x, b, y, d);
		mpfree(a);
		mpfree(b);
	}
